    ...
    **Arguments**
    ...
    def call(self, inputs):
        ...
        if mode == modes.SINGLE and K.is_sparse(A):
            ...
            output, attn_coef = self._call_dense(X, A, add_self_loops, remove_attn, training=training, beta=beta)
    ...
    def _call_dense(self, X, A, add_self_loops=True, remove_attn=False, training=True, beta=1):
        shape = tf.shape(A)[:-1]
        if add_self_loops:
        ...
        if remove_attn:
            attn_coef_drop = tf.stack([A for i in range(self.attn_heads)], axis=-2)
            attn_coef = A
        else:
